...
class InceptionWeb(nn.Module):
    """
    对应论文 3.4.1: GoogleNet Inception 思想的轻量化实现
    使用多尺度卷积核 (1x1, 3x3, 5x5) 并行提取特征，增强对小目标(烟头)和大目标(纸箱)的捕获能力。
    """
    def __init__(self, c1, c2):
        super().__init__()
        c_ = c2 // 4  # 分流通道数
        # 分支1: 1x1 卷积 (降维/保留主要特征)
        ...
        # 分支3: 1x1 -> 5x5 (大感受野，用两个3x3代替5x5以减少参数量，符合轻量化)
        self.branch3 = nn.Sequential(
            Conv(c1, c_, 1, 1),
            Conv(c_, c_, 3, 1),
            Conv(c_, c_, 3, 1)
        )
        # 分支4: MaxPool -> 1x1 (保留纹理结构)
...
class C3_Inception(nn.Module):
    """
    对应论文: CSP 架构与 Inception 的融合 (EcoDetect-YOLO 核心组件)
    替换原始 YOLOv5 的 C3 模块。
    """
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__()
        ...
        # 这里把原版的 Bottleneck 换成了 InceptionWeb
        self.m = nn.Sequential(*(InceptionWeb(c_, c_) for _ in range(n)))
